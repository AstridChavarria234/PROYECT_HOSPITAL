...
public class Unidad {
	...
	public Paciente buscarPaciente(int pNumeroIdentificacion) {
		if (pacientes != null) { // SOBRA ESTA VALIDACION
					...
					if (pEdad >= 0) {
						if (pDiagnostico != null || pDiagnostico != "") {
							if (pDiasHospitalizacion >= 0) {
								...
								p = new Paciente(pNombrePaciente, pApellido, pNumeroIdentificacion, pEdad, pDiagnostico,
	...
	public Paciente darPacienteMayorEdad() {
		...
		if (pacientes != null) {
			...
			for (Paciente paciente : pacientes) {
				...
				if (paciente.darEdad() >= max.darEdad()) {
					max = paciente;
				}
			}
